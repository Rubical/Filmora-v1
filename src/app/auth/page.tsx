...
import { Container, Button, Box, Input, Typography } from "@mui/material"
import FacebookIcon from "@mui/icons-material/Facebook"
import LinkedInIcon from "@mui/icons-material/LinkedIn"
import { validateEmail, validatePassword } from "@/utils/validation"
import BackToMainBtn from "../../components/UI/Button/BackToMainBtn"
import { createUser, signInWithOAuth } from "@/supabase/supabase"
...
const AuthPage = () => {
	...
	const { setEmail, setName } = useActions()

	const handleFormChange = (type: "signUp" | "signIn") => {
		setAuthType(type)
	}

	const checkPassword = ({ target: { value } }: ChangeEvent<HTMLInputElement>) => {
	...
	const checkEmail = ({ target: { value } }: ChangeEvent<HTMLInputElement>) => {
	...
	const handleSignUp = async () => {
		const { session, user } = await createUser({ email, password, name })
		if (user && session) {
		}
	}
		...
		>
			...
			<Box
				sx={{
					...
					alignItems: "center",
					position: "absolute",
					bottom: { xs: authType === "signIn" ? "0px" : "calc(100% - 420px)", lg: "0" },
					width: { xs: "100%", lg: "65%" },
					height: { xs: "65%", lg: "100%" },
					...
					color: "lightgray",
					zIndex: "100",
					left: { xs: "0px", lg: authType === "signIn" ? "0px" : "calc(100% - 800px)" },
					visibility: authType === "signIn" ? "hidden" : "visible",
					opacity: authType === "signIn" ? "0" : "1"
				}}
			>
				<Box
					sx={{
			...
			<Box
				sx={{
					...
					alignItems: "center",
					position: "absolute",
					bottom: { xs: authType === "signIn" ? "0px" : "calc(100% - 600px)", lg: "0" },
					width: { xs: "100%", lg: "65%" },
					height: { xs: "65%", lg: "100%" },
					padding: "25px",
					backgroundColor: "#1e1e1e",
					transition: "1.25s",
					color: "lightgray",
					left: { xs: "0px", lg: authType === "signIn" ? "0px" : "calc(100% - 600px)" },
					zIndex: authType === "signIn" ? "200" : "0",
					transformOrigin: "right",
					visibility: authType === "signIn" ? "visible" : "hidden",
					opacity: authType === "signIn" ? "1" : "0"
				}}
			>
				<Box
					sx={{
